    ...
    <SimpleRoleGuard roles={['veterinario']}>
      <div className="space-y-6">
        ...
        <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 sm:p-6 md:p-8">
          ...
          <div className="absolute top-0 right-0 w-64 h-64 sm:w-96 sm:h-96 bg-purple-500/20 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-64 h-64 sm:w-96 sm:h-96 bg-purple-600/20 rounded-full blur-3xl" />
          
          <div className="relative">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2 sm:gap-3 mb-2">
                  <Bell className="w-6 h-6 sm:w-8 sm:h-8 text-purple-400 flex-shrink-0" />
                  <h1 className="text-2xl sm:text-3xl font-bold text-white truncate">Notificaciones</h1>
                </div>
                <p className="text-slate-300 text-sm sm:text-base md:text-lg">Alertas médicas y novedades</p>
              </div>
              <button
          ...
          <CardContent>
            <div className="space-y-3">
              {filteredNotificaciones.map((notif) => (
                <div
                  key={notif.id}
                  className={`p-4 rounded-lg border ${notif.leido ? 'bg-gray-50 border-gray-200' : 'bg-purple-50 border-purple-200'}`}
                >
