...
import { tareaService, Tarea } from '@/lib/services/taskService';
...
export default function EditarTareaPage() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const [tareaResp, caballosResp, usuariosResp] = await Promise.all([
        ...
        caballoService.getAll({ page: 1, limit: 100 }),
        userService.getAll({ page: 1, limit: 100 })
      ]);

      setTarea(tareaResp);
      const horsesArr = (caballosResp as any)?.data?.caballos || (caballosResp as any)?.data || [];
      setCaballos(Array.isArray(horsesArr) ? horsesArr : []);
      const usersArr = Array.isArray(usuariosResp) ? usuariosResp : ((usuariosResp as any)?.data || []);
      ...
      router.push('/propietario/horses');
    } finally {
  ...
  if (loadingData) return (
    <div className="space-y-3">
      {[1,2,3,4].map(i => <div key={i} className="h-16 bg-gray-100 rounded-md animate-pulse" />)}
    </div>
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-gray-400 resize-none"
        ...
        <div className="flex items-center justify-end gap-4">
          <button
            ...
            disabled={loading}
            className="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 flex items-center gap-2 disabled:opacity-50"
          >
            <X className="w-4 h-4" />
            Cancelar
          </button>
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-2.5 bg-gray-900 text-white rounded-md hover:bg-gray-700 flex items-center gap-2 disabled:opacity-50"
          >
            {loading ? (
              <>
