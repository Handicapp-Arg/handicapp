...
import { AdjuntoService, type Adjunto } from '@/lib/services/adjuntoService';
import {
  DocumentTextIcon,
  ArrowDownTrayIcon,
  TrashIcon,
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900 flex items-center gap-3">
              <div className="p-2 bg-gradient-to-br from-[#0f172a]/10 to-[#af936f]/10 rounded-lg">
                <FolderIcon className="w-6 h-6 text-[#0f172a]" />
              </div>
              Documentos Adjuntos
              <span className="inline-flex items-center justify-center px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold">
      ...
      {adjuntosFiltrados.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {adjuntosFiltrados.map((adjunto) => (
            <div
              key={adjunto.id}
              className="group relative bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 hover:border-[#0f172a]/20 overflow-hidden"
            >
              {/* Preview de imagen o icono */}
              <div className="relative h-48 bg-gradient-to-br from-amber-50 to-amber-100 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center">
                {adjunto.tipo_mime.startsWith('image/') ? (
                  <div className="relative w-full h-full">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 p-2 bg-white/90 dark:bg-gray-900/90 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"
                    >
              ...
              <div className="p-4 space-y-3">
                <div>
                  <h4 className="font-medium text-gray-900 dark:text-white truncate" title={adjunto.nombre_archivo}>
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-100">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 bg-[#0f172a] hover:bg-[#0f172a]/90 text-white rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl text-sm font-medium"
                  >
                  ...
                  {showActions && (
                    <button
                      ...
                      className="p-2.5 bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 rounded-lg transition-all duration-200 hover:shadow"
                      title="Eliminar"
                    >
