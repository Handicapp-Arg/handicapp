...
export function NotificacionesPage() {
  ...
  const handleMarkAsRead = async (id: number) => {
    try {
      ...
      // Invalidar queries para actualizar el contador en la campanita
      await queryClient.invalidateQueries({ queryKey: ['notificaciones'] });
      await queryClient.invalidateQueries({ queryKey: ['notificaciones', 'contador'] });
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      // Invalidar queries para actualizar la lista y el contador
      await queryClient.invalidateQueries({ queryKey: ['notificaciones'] });
      await queryClient.invalidateQueries({ queryKey: ['notificaciones', 'contador'] });
    } catch (error) {
      console.error('Error:', error);
