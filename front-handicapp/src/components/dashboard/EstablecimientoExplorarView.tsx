...
export function EstablecimientoExplorarView() {
  ...
  const [filters, setFilters] = useState({
    tipo_establecimiento: undefined as string | undefined,
  ...
  const loadEstablecimientos = async () => {
    try {
      ...
      const response = await establecimientoService.getAll({
        ...
        sortOrder: 'ASC',
        search: searchTerm,
        tipo_establecimiento: filters.tipo_establecimiento,
        estado: filters.estado || 'activo' // Por defecto solo activos
      });
      ...
      if (Array.isArray(response)) {
        ...
        const r = response as { data?: unknown; items?: unknown; pagination?: { pages?: number }; [key: string]: unknown };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {/* Búsqueda, filtro y botón en una fila */}
      ...
      {establecimientos.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <p className="text-gray-600">
            {searchTerm ? 'No se encontraron establecimientos' : 'No hay establecimientos disponibles'}
          </p>
        </div>
      ) : (
        <>
